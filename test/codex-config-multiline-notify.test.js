...
test("upsertNotify replaces multi-line notify blocks without leaving trailing lines", async () => {
  ...
  const newNotify = ["/usr/bin/env", "node", "/Users/victor/.vibedeck/bin/notify.cjs"];
  ...
  assert.equal(
    updated.includes(
      'notify = [\"/usr/bin/env\", \"node\", \"/Users/victor/.vibedeck/bin/notify.cjs\"]',
    ),
    true,
  );
  assert.equal(
...
test("restoreNotify restores from notifyOriginalPath even if config was updated", async () => {
  ...
  const expectedNotify = ["/usr/bin/env", "node", "/Users/victor/.vibedeck/bin/notify.cjs"];
