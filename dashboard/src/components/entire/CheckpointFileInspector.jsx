...
export function CheckpointFileInspector({ file = null, loading = false, error = "", selectedPath = "", className = "" }) {
  ...
  if (!file) {
    return (
      <div className={cn("flex h-full min-h-0 max-h-full items-center overflow-hidden rounded-2xl border border-dashed border-oai-gray-300 bg-white/70 p-5 text-sm text-oai-gray-500 shadow-[0_20px_50px_rgba(15,23,42,0.04)] backdrop-blur dark:border-oai-gray-700 dark:bg-oai-gray-900/60 dark:text-oai-gray-400", className)}>
  ...
  return (
    <section className={cn("grid h-full min-h-0 max-h-full grid-rows-[auto_auto_minmax(0,1fr)] overflow-hidden rounded-2xl border border-[var(--glass-border)] bg-[var(--glass-bg)] backdrop-blur-[var(--glass-blur)] shadow-glass", className)}>
      ...
      <div className="flex min-h-0 items-center justify-between gap-3 border-b border-oai-gray-200 px-4 py-2 dark:border-oai-gray-800">
      ...
      <div className="min-h-0 overflow-hidden p-3">
        <div className="h-full min-h-0 overflow-auto rounded-md bg-oai-black/[0.03] p-3 dark:bg-white/[0.08]">
          {tab === "raw" ? (
