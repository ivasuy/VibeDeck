const REDIRECT_STORAGE_KEY = "vibedeck.dashboard.redirect.v1";
const POST_AUTH_PATH_STORAGE_KEY = "vibedeck.dashboard.post_auth_path.v1";
...
export function validateNextPath(value: any) {
  ...
  try {
    const url = new URL(trimmed, "https://vibedeck.local");
    if (url.origin !== "https://vibedeck.local") return null;
