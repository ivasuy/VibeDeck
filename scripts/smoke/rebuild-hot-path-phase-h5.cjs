...
const ARTIFACT_DIR = path.join(
  ...
  'superpowers',
  'plans',
  'phase-h5-smoke-artifacts',
);
const H4_BASELINE_MS = 284_982;
const H4_BASELINE_REPAIR_PASS_MS = 36_400.351;
const H4_BASELINE_BRANCH_FACT_STAGE_MS = 28_946.879;
const TARGET_MS = 300_000;
const REPAIR_PASS_STAGE_NAME = 'repair_pass';
const BRANCH_FACT_STAGE_NAME = 'branch_fact_rebuild_pass';
const PHASE_H5_ENV_FLAGS = {
...
function buildPhaseH5SmokeSummary({
  profile = null,
  wallClockMs,
  command,
  exitCode = 0,
  ...
  const repairPassGate = stageGate({
    ...
    baselineMs: H4_BASELINE_REPAIR_PASS_MS,
  });
  const branchFactGate = stageGate({
    exitCode,
    stages,
    stageName: BRANCH_FACT_STAGE_NAME,
    baselineMs: H4_BASELINE_BRANCH_FACT_STAGE_MS,
  });
  ...
  return {
    ...
    command,
    phase_h5_flags: PHASE_H5_ENV_FLAGS,
    phase_h4_baseline: {
      wall_clock_ms: H4_BASELINE_MS,
      repair_pass_stage_name: REPAIR_PASS_STAGE_NAME,
      repair_pass_ms: H4_BASELINE_REPAIR_PASS_MS,
      branch_fact_stage_name: BRANCH_FACT_STAGE_NAME,
      branch_fact_stage_ms: H4_BASELINE_BRANCH_FACT_STAGE_MS,
    },
    ...
    wall_clock_gate: {
      baseline_ms: H4_BASELINE_MS,
      target_ms: TARGET_MS,
      current_ms: measuredMs,
      passed: wallClockPassed,
      delta_vs_baseline_ms: measuredMs === null ? null : measuredMs - H4_BASELINE_MS,
...
async function writePhaseH5SmokeArtifacts({
  ...
  const copiedProfilePath = path.join(artifactDir, 'phase-h5-rebuild-profile.json');
  const summaryPath = path.join(artifactDir, 'phase-h5-rebuild-summary.json');
  await copyIfPresent(profilePath, copiedProfilePath);
  const profile = await readJsonIfPresent(profilePath);
  const summary = buildPhaseH5SmokeSummary({
    profile,
    wallClockMs,
    command,
    exitCode,
...
async function runPhaseH5Rebuild() {
...
async function main() {
  const run = await runPhaseH5Rebuild();
  const error =
    run.exitCode === 0
      ? null
      : {
          message: 'phase h5 rebuild command exited non-zero',
          stdout_tail: run.stdout_tail,
          stderr_tail: run.stderr_tail,
        };
  const summary = await writePhaseH5SmokeArtifacts({
...
module.exports = {
  H4_BASELINE_MS,
  H4_BASELINE_REPAIR_PASS_MS,
  H4_BASELINE_BRANCH_FACT_STAGE_MS,
  TARGET_MS,
  PHASE_H5_ENV_FLAGS,
  buildPhaseH5SmokeSummary,
  writePhaseH5SmokeArtifacts,
};
