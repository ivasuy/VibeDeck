...
function tmpFile(initial) {
  const f = path.join(fs.mkdtempSync(path.join(os.tmpdir(), 'vd-codebuddy-')), 'settings.json');
...
test('5. malformed JSON aborts and never overwrites', async () => {
  ...
  await assert.rejects(() => codebuddy.install(f));
