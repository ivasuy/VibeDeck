...
const ARTIFACT_DIR = path.join(
  ...
  'superpowers',
  'plans',
  'phase-h6-smoke-artifacts',
);
const H5_BASELINE_MS = 285_133;
const H5_BASELINE_REPAIR_PASS_MS = 37_710.939;
const H5_BASELINE_BRANCH_FACT_STAGE_MS = 25_900.448;
const TARGET_MS = 300_000;
const REPAIR_PASS_STAGE_NAME = 'repair_pass';
const BRANCH_FACT_STAGE_NAME = 'branch_fact_rebuild_pass';
const PHASE_H6_ENV_FLAGS = {
...
function stageGate({ exitCode, stages, stageName, baselineMs }) {
  const currentMs = stageDuration(stages, stageName);
  const covered = currentMs !== null;
  const passed = exitCode === 0 && covered && currentMs <= baselineMs;
...
function buildPhaseH6SmokeSummary({
  profile = null,
  wallClockMs,
  command,
  exitCode = 0,
  ...
  const repairPassGate = stageGate({
    ...
    baselineMs: H5_BASELINE_REPAIR_PASS_MS,
  });
  const branchFactGate = stageGate({
    exitCode,
    stages,
    stageName: BRANCH_FACT_STAGE_NAME,
    baselineMs: H5_BASELINE_BRANCH_FACT_STAGE_MS,
  });
  ...
  return {
    ...
    command,
    phase_h6_flags: PHASE_H6_ENV_FLAGS,
    phase_h5_baseline: {
      wall_clock_ms: H5_BASELINE_MS,
      repair_pass_stage_name: REPAIR_PASS_STAGE_NAME,
      repair_pass_ms: H5_BASELINE_REPAIR_PASS_MS,
      branch_fact_stage_name: BRANCH_FACT_STAGE_NAME,
      branch_fact_stage_ms: H5_BASELINE_BRANCH_FACT_STAGE_MS,
    },
    ...
    wall_clock_gate: {
      baseline_ms: H5_BASELINE_MS,
      target_ms: TARGET_MS,
      current_ms: measuredMs,
      passed: wallClockPassed,
      delta_vs_baseline_ms: measuredMs === null ? null : measuredMs - H5_BASELINE_MS,
...
async function writePhaseH6SmokeArtifacts({
  ...
  const copiedProfilePath = path.join(artifactDir, 'phase-h6-rebuild-profile.json');
  const summaryPath = path.join(artifactDir, 'phase-h6-rebuild-summary.json');
  await copyIfPresent(profilePath, copiedProfilePath);
  const profile = await readJsonIfPresent(profilePath);
  const summary = buildPhaseH6SmokeSummary({
    profile,
    wallClockMs,
    command,
    exitCode,
...
async function runPhaseH6Rebuild() {
...
async function main() {
  const run = await runPhaseH6Rebuild();
  const error =
    run.exitCode === 0
      ? null
      : {
          message: 'phase h6 rebuild command exited non-zero',
          stdout_tail: run.stdout_tail,
          stderr_tail: run.stderr_tail,
        };
  const summary = await writePhaseH6SmokeArtifacts({
...
module.exports = {
  H5_BASELINE_MS,
  H5_BASELINE_REPAIR_PASS_MS,
  H5_BASELINE_BRANCH_FACT_STAGE_MS,
  TARGET_MS,
  PHASE_H6_ENV_FLAGS,
  buildPhaseH6SmokeSummary,
  writePhaseH6SmokeArtifacts,
};
