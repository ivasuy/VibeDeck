...
const ARTIFACT_DIR = path.join(
  ...
  'superpowers',
  'plans',
  'phase-h2-smoke-artifacts',
);
const H1_BASELINE_MS = 368_366;
const H1_BASELINE_FLUSH_STAGE_MS = 309_554.01;
const H1_BASELINE_FLUSH_COUNT = 10;
const TARGET_MS = 300_000;
const TARGET_FLUSH_STAGE_MS = 180_000;
const TARGET_FLUSH_COUNT = 120;
const FLUSH_STAGE_NAME = 'recent_lane_session_event_flush';
const PHASE_H2_ENV_FLAGS = {
...
function buildPhaseH2SmokeSummary({
  profile = null,
  wallClockMs,
  command,
  exitCode = 0,
  ...
  const gatePassed = performancePassed && flushStagePassed && flushCountPassed;

  return {
    generated_at: generatedAt,
    command,
    phase_h2_flags: PHASE_H2_ENV_FLAGS,
    phase_h1_baseline: {
      wall_clock_ms: H1_BASELINE_MS,
      flush_stage_name: FLUSH_STAGE_NAME,
      flush_stage_ms: H1_BASELINE_FLUSH_STAGE_MS,
      flush_count: H1_BASELINE_FLUSH_COUNT,
    },
...
async function writePhaseH2SmokeArtifacts({
  ...
  const copiedProfilePath = path.join(artifactDir, 'phase-h2-rebuild-profile.json');
  const summaryPath = path.join(artifactDir, 'phase-h2-rebuild-summary.json');
  await copyIfPresent(profilePath, copiedProfilePath);
  const profile = await readJsonIfPresent(profilePath);
  const summary = buildPhaseH2SmokeSummary({
    profile,
    wallClockMs,
    command,
    exitCode,
...
async function runPhaseH2Rebuild() {
...
async function main() {
  const run = await runPhaseH2Rebuild();
  const error =
    run.exitCode === 0
      ? null
      : {
          message: 'phase h2 rebuild command exited non-zero',
          stdout_tail: run.stdout_tail,
          stderr_tail: run.stderr_tail,
        };
  const summary = await writePhaseH2SmokeArtifacts({
...
module.exports = {
  H1_BASELINE_MS,
  H1_BASELINE_FLUSH_STAGE_MS,
  H1_BASELINE_FLUSH_COUNT,
  TARGET_MS,
  TARGET_FLUSH_STAGE_MS,
  TARGET_FLUSH_COUNT,
  PHASE_H2_ENV_FLAGS,
  buildPhaseH2SmokeSummary,
  writePhaseH2SmokeArtifacts,
};
