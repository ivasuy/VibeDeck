...
function tmpFile(initial) {
  const f = path.join(fs.mkdtempSync(path.join(os.tmpdir(), 'vd-gemini-')), 'settings.json');
  if (initial != null) fs.writeFileSync(f, initial);
  return f;
}

test('1. empty file: install adds vibedeck entry and enables hooks', async () => {
  const f = tmpFile(null);
  await gemini.install(f);
  const json = JSON.parse(fs.readFileSync(f, 'utf8'));
  assert.strictEqual(json.hooks.SessionEnd.filter((e) => e && e._vibedeck === 'v1').length, 1);
  assert.strictEqual(json.tools.enableHooks, true);
});
...
test('5. malformed JSON aborts and never overwrites', async () => {
  ...
  await assert.rejects(() => gemini.install(f));
