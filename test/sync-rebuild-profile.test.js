...
test('phase h4 smoke summary records repair-pass dedupe benchmark gates', async () => {
  const tmp = await fs.mkdtemp(path.join(os.tmpdir(), 'vd-phase-h4-smoke-test-'));
  try {
    const profilePath = path.join(tmp, 'rebuild_profile.json');
    await fs.writeFile(
      profilePath,
    ...
    const smokeHarness = require('../scripts/smoke/rebuild-hot-path-phase-h4.cjs');
    const summary = await smokeHarness.writePhaseH4SmokeArtifacts({
      artifactDir: tmp,
      profilePath,
      wallClockMs: 299_000,
      command: 'node bin/vibedeck.js sync --rebuild-vibedeck-db',
    ...
    assert.equal(summary.result, 'pass');
    assert.equal(summary.wall_clock_gate.baseline_ms, 337_412);
    ...
    assert.equal(summary.repair_pass_gate.baseline_ms, 65_127.053);
    assert.equal(summary.repair_pass_gate.current_ms, 25_000);
    ...
    assert.equal(summary.branch_fact_gate.baseline_ms, 27_321.343);
    assert.equal(summary.branch_fact_gate.current_ms, 27_000);
    assert.equal(summary.branch_fact_gate.covered, true);
    assert.equal(summary.branch_fact_gate.passed, true);

    const copiedProfile = JSON.parse(
      await fs.readFile(path.join(tmp, 'phase-h4-rebuild-profile.json'), 'utf8'),
    );
    assert.equal(copiedProfile.counters.branch_facts_rebuilt_by_scope.dirty, 2);

    const writtenSummary = JSON.parse(
      await fs.readFile(path.join(tmp, 'phase-h4-rebuild-summary.json'), 'utf8'),
    );
    assert.equal(writtenSummary.repair_pass_gate.current_ms, 25_000);
  } finally {
    await fs.rm(tmp, { recursive: true, force: true });
  }
});

test('phase h4 smoke summary marks missing key stages uncovered and failed', async () => {
  const tmp = await fs.mkdtemp(path.join(os.tmpdir(), 'vd-phase-h4-smoke-missing-stage-'));
  try {
    const profilePath = path.join(tmp, 'rebuild_profile.json');
    await fs.writeFile(
      profilePath,
...
test('phase h6 smoke summary records wall-clock repair-pass and branch-fact gates', async () => {
  const tmp = await fs.mkdtemp(path.join(os.tmpdir(), 'vd-phase-h6-smoke-test-'));
  try {
    const profilePath = path.join(tmp, 'rebuild_profile.json');
    await fs.writeFile(
      profilePath,
    ...
    const smokeHarness = require('../scripts/smoke/rebuild-hot-path-phase-h6.cjs');
    const summary = await smokeHarness.writePhaseH6SmokeArtifacts({
      artifactDir: tmp,
      profilePath,
      wallClockMs: 299_000,
      command: 'node bin/vibedeck.js sync --rebuild-vibedeck-db',
    ...
    assert.equal(summary.result, 'pass');
    assert.equal(summary.wall_clock_gate.baseline_ms, 285_133);
    assert.equal(summary.wall_clock_gate.target_ms, 300_000);
    assert.equal(summary.wall_clock_gate.current_ms, 299_000);
    assert.equal(summary.wall_clock_gate.passed, true);
    assert.equal(summary.repair_pass_gate.stage_name, 'repair_pass');
    assert.equal(summary.repair_pass_gate.baseline_ms, 37_710.939);
    assert.equal(summary.repair_pass_gate.current_ms, 37_000);
    assert.equal(summary.repair_pass_gate.covered, true);
    assert.equal(summary.repair_pass_gate.passed, true);
    assert.equal(summary.branch_fact_gate.stage_name, 'branch_fact_rebuild_pass');
    assert.equal(summary.branch_fact_gate.baseline_ms, 25_900.448);
    assert.equal(summary.branch_fact_gate.current_ms, 25_000);
    assert.equal(summary.branch_fact_gate.covered, true);
    assert.equal(summary.branch_fact_gate.passed, true);

    const copiedProfile = JSON.parse(
      await fs.readFile(path.join(tmp, 'phase-h6-rebuild-profile.json'), 'utf8'),
    );
    assert.equal(copiedProfile.counters.branch_facts_rebuilt_by_scope.dirty, 2);

    const writtenSummary = JSON.parse(
      await fs.readFile(path.join(tmp, 'phase-h6-rebuild-summary.json'), 'utf8'),
    );
    assert.equal(writtenSummary.repair_pass_gate.current_ms, 37_000);
    assert.equal(writtenSummary.branch_fact_gate.current_ms, 25_000);
  } finally {
    await fs.rm(tmp, { recursive: true, force: true });
  }
});

test('phase h6 smoke summary marks missing key stages uncovered and failed', async () => {
  const tmp = await fs.mkdtemp(path.join(os.tmpdir(), 'vd-phase-h6-smoke-missing-stage-'));
  try {
    const profilePath = path.join(tmp, 'rebuild_profile.json');
    await fs.writeFile(
      profilePath,
