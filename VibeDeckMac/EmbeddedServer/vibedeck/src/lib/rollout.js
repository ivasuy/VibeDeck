...
function extractClaudeCwdFromLine(line) {
  if (typeof line !== "string" || !line.includes('"cwd"')) return null;

  let match = line.match(/"cwd"\s*:\s*"([^"]+)"/);
  if (match && match[1]) return match[1];

  match = line.match(/\\"cwd\\":\\"([^"]+)\\"/);
  if (match && match[1]) {
    try {
...
function resolveOmpHome(env = process.env) {
  const home = env.HOME || require("node:os").homedir();
  // Honor VibeDeck override first, then oh-my-pi upstream env vars.
...
// default directory.
//
// Token semantics map directly onto VibeDeck conventions: `inputTokens`
