...
test('resolveBranchForSession: B hit returns B/medium', async () => {
  const { resolveBranchForSession } = require('../src/lib/sessions/resolve-branch');
  const r = await resolveBranchForSession(
    {
      provider: 'codex',
...
test('resolveBranchForSession: B null and C hit returns C/low', async () => {
  const { resolveBranchForSession } = require('../src/lib/sessions/resolve-branch');
  const r = await resolveBranchForSession(
    {
      provider: 'codex',
...
test('resolveBranchForSession: manual override returns OVERRIDE/high and ignores all tiers', async () => {
  ...
  assert.deepEqual(r, { branch: 'manual', tier: 'OVERRIDE', confidence: 'high' });
  assert.deepEqual(calls, []);
});
